  ...
  function handleSubmit(formData) 
  {
    ...
    const terms = formData.get('terms-and-conditions');
     ...
     if(isEmail(email)===false){
      ...
      if(hasMinLength(password,8)===false||isNotEmpty(password)===false){ 
        error.push('Password must be at least 8 characters long');
      }
      if(isEqualToOtherValue(password,confirmPassword)===false){
        error.push('Passwords do not match');
      }
      if(isNotEmpty(firstName)===false||isNotEmpty(lastName)===false){
        error.push('First name and Last name cannot be empty');
      }
      if(isNotEmpty(role)===false){
